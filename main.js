...
var DailyScoringModal = class extends import_obsidian.Modal {
      ...
      if (this.plugin.settings.items.length === 0) {
        ...
        for (var cat of categories) {
          ...
          if (catItems.length > 0) {
            ...
            itemsContainer.createEl("h3", { text: cat, cls: "kid-score-section-title" });
  ...
  insertAttachment(label, ext) {
    ...
    document.body.appendChild(overlay); fileInput.focus();
  }
  ...
  renderCard(item, grid, yesterdayData) {
    ...
    if (yesterdayData) {
      var yScore = yesterdayData.scores[item.id]; var yEarned = yScore !== undefined && yScore > 0;
      card.createDiv({ cls: "card-yesterday " + (yEarned ? "was-earned" : "was-missed"), text: yEarned ? "\u6628\u2705" : "\u6628\u274C" });
    }
    var pressTimer = null; var isLongPress = false;
    card.addEventListener("pointerdown", function() { isLongPress = false; pressTimer = setTimeout(function() { isLongPress = true; self.showCustomValuePopup(item, function(v) { self.scores[item.id] = v; self.refreshCard(card, item); self.updateTotalDisplay(); }); }, 500); });
    card.addEventListener("pointerup", function() { clearTimeout(pressTimer); if (!isLongPress) { self.scores[item.id] = self.scores[item.id] === 0 ? item.points : 0; self.refreshCard(card, item); self.updateTotalDisplay(); } });
    card.addEventListener("pointercancel", function() { clearTimeout(pressTimer); });
  ...
  showCustomValuePopup(item, callback) {
    ...
    var overlay = document.createElement("div"); overlay.className = "kid-score-value-overlay";
    var popup = document.createElement("div"); popup.className = "kid-score-value-popup";
    popup.innerHTML = '<div class="value-popup-header">' + item.emoji + " " + item.name + '</div><div class="value-popup-hint">\u9ED8\u8BA4\u5206\u503C\uFF1A' + (item.points >= 0 ? "+" : "") + item.points + "</div>";
  ...
  showAddCustomItemForm() {
    ...
    emojiPickBtn.onclick = function() { showEmojiPicker(function(em) { emojiInput.value = em; }); };
    ...
    var nameInput = document.createElement("input"); nameInput.type = "text"; nameInput.className = "custom-form-name-input"; nameInput.placeholder = "\u4E8B\u9879\u540D\u79F0...";
    ...
    acts.appendChild(cb); acts.appendChild(ok); popup.appendChild(acts);
    overlay.appendChild(popup); overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
    document.body.appendChild(overlay); nameInput.focus();
  }
