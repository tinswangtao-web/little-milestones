...
- **Review AI** 每次 review 结束后，除审查结论外，必须额外产出一段**可直接发送给代码 AI 的下一步内容**：若发现问题，给出可直接转发的最小修复指令；若无明确修复项，给出可直接转发的下一步建议（如测试、同步 Vault、收口、提交前准备等）。此要求对任何承担 review 角色的 AI 一律适用。
...
- **Review AI** 输出末尾必须附一段“给代码 AI / 下一步”的可执行文字：有修复项时写成可直接转发给代码 AI 的指令；无修复项时写成下一步建议或收口建议。
