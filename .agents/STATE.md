...
- `awaiting`: user
- `handoff-note`: **Role redefinition per user request**: codex is the sole implementation agent for this plugin. `claude-code` and `kimi-code` are review-only by default and may suggest fixes, but they should not edit plugin code unless the user explicitly authorizes that exception and the exception is recorded here first. Codex has now also refined the diary module fields: multiline daily-record textareas auto-grow instead of showing internal scrollbars, selected weather/mood values wrap in an auto-growing textarea, and the custom weather/mood add row uses a roomier two-line layout so long text is fully visible on desktop and iPhone. Rebuilt artifacts were re-synced to Vault with hash verification. Next: user validates on real devices; any further fixes should be assigned to codex for implementation, then other AIs may review.
