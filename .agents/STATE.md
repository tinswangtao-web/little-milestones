...
- `awaiting`: user
- `handoff-note`: Codex continued the Kimi mobile-UX review with a second narrow pass: emoji picker now blurs the active input before opening so the iOS keyboard can get out of the way; `setupModalKeyboard()` cleanup now restores prior inline styles instead of blindly clearing them; extra-small mobile date navigation now forces ellipsis/overflow handling. Rebuilt artifacts and re-synced to Vault with hash verification.
